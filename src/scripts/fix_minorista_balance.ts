...
async function run() {
    try {
        ...
        console.log(`Current State: CreditLimit=${minorista.creditLimit}, Available=${minorista.availableCredit}, CreditBalance=${minorista.creditBalance}`);
        ...
        if (transactions.length > 0) {
            ...
            console.log(`Last Transaction Stored State: Available=${lastTx.availableCredit}, CreditBalance=${lastTx.currentBalanceInFavor}`);

            if (Math.abs(lastTx.availableCredit - minorista.availableCredit) > 1) {
                console.log(`MISMATCH DETECTED! Minorista entity has ${minorista.availableCredit}, but last transaction says ${lastTx.availableCredit}.`);
