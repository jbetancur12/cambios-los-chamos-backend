...
async function auditMinorista() {
  ...
  try {
    ...
    if (dateStart) {
      ...
      if (initialBalance < 0)
        console.log(
          `(Es decir, comenzó el día con DEUDA de $${Math.abs(initialBalance).toLocaleString('es-VE', { minimumFractionDigits: 2 })})`
        )
    ...
    const transactions = await em.find(MinoristaTransaction, where, { orderBy: { createdAt: 'ASC' } })
    ...
    if (dateStart)
      console.log(
        `Saldo Inicial (${dateStr}):      $${initialBalance.toLocaleString('es-VE', { minimumFractionDigits: 2 })}`
      )
    ...
    console.log(
      `Total Reembolsos (REFUND):     $${totalReembolsos.toLocaleString('es-VE', { minimumFractionDigits: 2 })}`
    )

    console.log('\n--- Balance del Periodo Seleccionado ---')
    console.log(
      `Movimiento Neto del Día:       $${periodBalance.toLocaleString('es-VE', { minimumFractionDigits: 2 })}`
    )
    ...
    if (finalBalance >= 0) {
      console.log(
        `RESULTADO FINAL: SALDO A FAVOR de $${finalBalance.toLocaleString('es-VE', { minimumFractionDigits: 2 })}`
      )
    } else {
      console.log(
        `RESULTADO FINAL: DEUDA de $${Math.abs(finalBalance).toLocaleString('es-VE', { minimumFractionDigits: 2 })}`
      )
    }
  } catch (error) {
