 ...
 * Get all amounts for a specific operator (including inactive) - SUPER_ADMIN only
 */
router.get('/:operatorId/all', requireRole(UserRole.SUPER_ADMIN), async (req: Request, res: Response) => {
  try {
    const { operatorId } = req.params
 ...
 * Create a new operator-amount relation - SUPER_ADMIN only
 */
