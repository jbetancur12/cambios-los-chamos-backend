...
bankAccountRouter.post('/create', requireRole(UserRole.SUPER_ADMIN, UserRole.ADMIN, UserRole.TRANSFERENCISTA), validateBody(createBankAccountSchema), async (req: Request, res: Response) => {
    ...
    if ('error' in result) {
        switch (result.error) {
            ...
            case 'ACCOUNT_NUMBER_EXISTS':
                return res.status(409).json(ApiResponse.conflict('Ya existe una cuenta con este número para este transferencista'))
        }
    }
...
// ------------------ ACTUALIZAR BALANCE ------------------
bankAccountRouter.patch('/:bankAccountId/balance', requireRole(UserRole.SUPER_ADMIN, UserRole.ADMIN), validateBody(updateBankAccountBalanceSchema), async (req: Request, res: Response) => {
    const { bankAccountId } = req.params
